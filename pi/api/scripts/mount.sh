...
# TODO make proper attribution for mount.sh, unmount.sh and usb.rules
...
echo "Got device information: $DEVNAME" >> /usr/src/mount.log
...
if [ "$ID_FS_TYPE" = "exfat" ]; then
        echo "EXFAT - ignoring fs type check" >> /usr/src/mount.log
else
        echo "not EXFAT $ID_FS_TYPE" >> /usr/src/mount.log
