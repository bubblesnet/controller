...
wsServer.locals = {};
wsServer.locals.config = require('./config/locals.js');
wsServer.locals.units = require('./api/services/formatted_units.js');

// view engine setup
wsServer.set('views', path.join(__dirname, 'views'));
wsServer.set('view engine', 'pug');

wsServer.use(function (req, res, next) {
...
wsServer.use(bodyParser.urlencoded({
    extended: true
}));

wsServer.locals.moment = require('moment');
wsServer.locals.sprintf = require("sprintf-js").sprintf;
wsServer.locals.config = require('./config/locals.js');
wsServer.locals.units = require('./api/services/formatted_units.js');
...
//wsServer.use(favicon(path.join(__dirname, 'public', 'favicon.ico')));
wsServer.use(logger('dev'));
wsServer.use(bodyParser.json());
wsServer.use(bodyParser.urlencoded({extended: false}));
wsServer.use(cookieParser());
wsServer.use(express.static(path.join(__dirname, 'public')));

wsServer.use('/api/users', user_routes);
wsServer.use('/api/auth', auth_routes);
wsServer.use('/api/video', video_routes);
wsServer.use("/api/edgecontrol", edgecontrol_routes);
wsServer.use("/api/measurement", edgemeasurement_routes);
wsServer.use("/api/icebreaker", icebreaker_routes);
//wsServer.use('/', index);
...
// error handler
wsServer.use(function (err, req, res, next) {
